variable "private_endpoints" {
  ...
  validation {
    condition = alltrue([
      ...
      length(trimspace(pe.target_resource_id)) > 0 &&
      length(pe.subresource_names) > 0 &&
      alltrue([for s in pe.subresource_names : length(trimspace(s)) > 0]) &&
      length(pe.private_dns_zone_ids) > 0 &&
      alltrue([for id in pe.private_dns_zone_ids : length(trimspace(id)) > 0])
    ])
    error_message = "Each private endpoint requires name, target_resource_id, non-empty subresource_names, and non-empty private_dns_zone_ids."
  }

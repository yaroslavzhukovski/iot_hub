...
variable "iot_hub_location" {
  description = "Azure region for IoT Hub (can differ from primary location due to subscription region restrictions)."
  type        = string
  default     = "westeurope"

  validation {
    condition     = length(trimspace(var.iot_hub_location)) > 0
    error_message = "iot_hub_location must not be empty."
  }
}
...
variable "iot_device_twin_map" {
  description = "Mapping from IoT device IDs to ADT twin IDs used by the Function app."
  type        = map(string)
  default     = {}

  validation {
    condition = alltrue([
      for k, v in var.iot_device_twin_map :
      length(trimspace(k)) > 0 && length(trimspace(v)) > 0
    ])
    error_message = "iot_device_twin_map keys and values must be non-empty strings."
  }
}

variable "iot_ingest_consumer_group_name" {
  description = "Consumer group used by Function to read IoT Hub built-in events endpoint."
  type        = string
  default     = "func-ingest"

  validation {
    condition     = length(trimspace(var.iot_ingest_consumer_group_name)) > 0 && !startswith(var.iot_ingest_consumer_group_name, "$")
    error_message = "iot_ingest_consumer_group_name must not be empty and must not start with '$'."
  }
}

variable "rbac_assignments" {
  description = "Map of RBAC assignments for this environment."
  type = map(object({
    scope                = string
    principal_id         = string
...
variable "log_analytics_daily_quota_gb" {
  description = "Daily quota in GB for Log Analytics (-1 means unlimited)."
  type        = number
  default     = 1
}
